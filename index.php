...
$.ajax({
    ...
    success: function(data) {
        ...
        aProposPerson.innerHTML=`<img id='aProposBack_2' src="image/aPropos/back_2.svg"/>
        <img id='aProposBack_1' src="image/aPropos/back_1.svg"/>
        <img id='aProposBack_3' src="image/aPropos/back_3.svg"/>
    ...
    success: function(data) {
        if (data.length > 0) {
            $.each(data.reverse(), function(i, val) {
                let title=val.title.slice(0,-3).toLowerCase().replaceAll(' ', '_').replaceAll('?', '').replaceAll('&', '').replaceAll('-', '_').replaceAll("'", '_')
                let inData
                if(val.description!==null){
                    inData='<a class="shot" target="_blank" href="'+ val.html_url +'" title="' + val.title.slice(0,-2) + '"><span class="fleche"></span><div class="title">' + val.title.slice(0,-2) + '</div><img src="'+ val.images.hidpi +'"/><p>'+val.description+'</p></a>'
                }else{
                    inData='<a class="shot" target="_blank" href="'+ val.html_url +'" title="' + val.title.slice(0,-2) + '"><span class="fleche"></span><div class="title">' + val.title.slice(0,-2) + '</div><img src="'+ val.images.hidpi +'"/></a>'
                }
